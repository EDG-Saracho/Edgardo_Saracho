...
void setup() {
  ...
  for (int thisNote = 0; thisNote < notes * 2; thisNote = thisNote + 2) {
    ...
    if (divider > 0) {
      // Nota regular, proceder
      noteDuration = (wholenote) / divider;
    } else if (divider < 0) {
      // Notas con puntillo representadas con duraciones negativas
      noteDuration = (wholenote) / abs(divider);
      noteDuration *= 1.5; // Aumenta la duración a la mitad para notas con puntillo
    }

    // Tocar la nota durante el 90% de la duración, dejando el 10% como pausa
    tone(buzzer, melody[thisNote], noteDuration * 0.9);

    // Esperar la duración especificada antes de tocar la siguiente nota
    delay(noteDuration);
    
    // Detener la generación de la forma de onda antes de la siguiente nota
    noTone(buzzer);
  }
}
